  ...
  <div class="details">
    ...
    <div class="content">
      ...
      <el-table :data="tableData" border style="width: 100%; margin-top: 20px">
        <el-table-column prop="日期" label="更新时间" width="120" align="center"></el-table-column>
        ...
        <el-table-column prop="类型" label="动态类型" width="100" align="center"></el-table-column>
        ...
        <el-table-column prop="分组" label="监测分组" width="120" align="center"></el-table-column>
      </el-table>
    </div>
    <el-dialog
      title="详情"
...
export default {
        ...
        shortcuts: [
          {
            text: "最近一周",
            onClick: picker => {
              const end = this.$store.state.nowDate;
  ...
  methods: {
    ...
    onSubmit() {
      if (
        this.formInline.region[this.formInline.region.length - 1] == "全部" ||
        this.formInline.region.length == 0
      ) {
        this.formInline.region = ["全部"];
      } else {
        this.formInline.region = this.formInline.region.filter(
          item => item != "全部"
        );
      }
      this.currentPage = 1;
  ...
  created() {
    ...
    if (this.date == 2592000) {
      this.value2 = [
        this.$store.state.nowDate - 30 * 86400000,
        this.$store.state.nowDate
      ];
    } else {
      this.value2 = [
        this.$store.state.nowDate - 7 * 86400000,
        this.$store.state.nowDate
      ];
    }
    this.into();
